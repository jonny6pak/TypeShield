...
// MARK: - Global tap refs so we can re-enable them
var kbTapRef: CFMachPort?
var ptTapRef: CFMachPort?

// MARK: - Taps with auto re-enable
...
// MARK: - Watchdog to keep taps enabled
...
// MARK: - Signal handling and run loop
signal(SIGINT) { _ in
    fputs("\nTypeShield: exiting…\n", stderr)
    if let kb = kbTapRef {
